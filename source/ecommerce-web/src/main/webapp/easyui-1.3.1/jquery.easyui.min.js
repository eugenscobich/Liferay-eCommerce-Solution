				...
				for ( var j = 0; j < cols.length; j++) {
					...
					if (col.checkbox) {
						...
						if (col.field) {
							...
							col.cellClass = "datagrid-cell-c" + _450 + "-" + col.field.replace(/\./g, "-").replace("[", "-").replace("]", "-");
	...
	var _6fa = $.extend({}, $.fn.datagrid.defaults.view, {
		...
		renderRow : function(_70e, _70f, _710, _711, row) {
			...
			for ( var i = 0; i < _70f.length; i++) {
				...
				if (col) {
					var _713 = col.styler ? (col.styler(getFieldValue(row, _712), row) || "") : "";
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _712 + "\" value=\"" + (getFieldValue(row, _712) != undefined ? getFieldValue(row, _712) : "") + "\"/>");
					} else {
						var val = null;
						if (col.formatter) {
							val = col.formatter(getFieldValue(row, _712), row);
						} else {
							val = getFieldValue(row, _712);
						}
